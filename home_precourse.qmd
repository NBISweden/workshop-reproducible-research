...
We will use Conda environments for the set up of several of the tutorials, but
don't worry if you don't understand exactly what everything does - you'll learn
all the details at the course.
...
stability and better performance, by running the following:
...
If you want to simplify this whole process you can use [this
script](https://github.com/fasterius/conda-intel-env).
:::
...
Quarto can also be installed using Conda. We will also install some
software packages that are used in the Quarto tutorial using Conda:

```bash
conda create --file env/conda-specfiles/quarto-env_<platform>_conda_spec.txt --name quarto-env
```

The valid platforms are `osx-64` (Intel-based Mac systems), `osx-arm64` (M-series Mac systems) and `linux-64` (Linux systems). So, for example, if you have a new Apple Silicon Mac you'd use `env/conda-specfiles/quarto-env_osx-arm64_conda_spec.txt` as the path after the `--file` flag.

## Installing Jupyter

Install Jupyter like so:

```bash
conda create --file env/conda-specfiles/jupyter-env_<platform>_conda_spec.txt --name jupyter-env
...
Create and activate the Conda environment with the commands below:

```bash
conda create --file env/conda-specfiles/snakemake-env_<platform>_conda_spec.txt --name snakemake-env
...
want to use Conda to create the environment:

```bash
conda create --file env/conda-specfiles/nextflow-env_<platform>_conda_spec.txt --name nextflow-env
...
If installing these tools locally is problematic, you can use [GitHub Codespaces](https://github.com/features/codespaces) to work in a cloud-based development environment with all the necessary tools pre-configured.

### Using GitHub Codespaces

1. Fork the [workshop repository](https://github.com/NBISweden/workshop-reproducible-research) to your GitHub account by pressing the **Fork** icon on the repository's page
2. Click the green **Code** button on your fork
3. Select the **Codespaces** tab and click **Create codespace on main**
4. Wait for the environment to build (this may take a few minutes on first launch)

Once the Codespace is ready, you'll have:
- Git, Conda, and Docker installed. The other environments can be created with Conda as described above. Alternatively, if you're familiar with Pixi, the environments are already available.
- A VS Code editor in your browser
- Pre-configured Conda channels (conda-forge and bioconda)
- All tutorial materials ready to use

You can then follow the tutorials as normal. GitHub provides [~120 compute hours per month](https://docs.github.com/en/billing/managing-billing-for-github-codespaces/about-billing-for-github-codespaces) free for personal accounts, which should be sufficient for completing this course.

::: {.callout-note}
You'll need a GitHub account to use Codespaces. See the [GitHub setup](#github-setup) section above if you don't already have one.
:::
