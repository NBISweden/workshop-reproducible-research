...
```{r, eval=TRUE, include=FALSE}
# Create the Ioannidis dataframe
df <- data.frame(
  result = c("...in principle", "...with some discrepancies", "...from processed data with some discrepancies", "...partially with some discrepancies", "Cannot reproduce"),
  value = c(2, 1, 4, 1, 10)
  )
df$result <- factor(df$result, levels=c("...in principle", "...with some discrepancies", "...from processed data with some discrepancies", "...partially with some discrepancies", "Cannot reproduce"))
```

```{r, eval=TRUE, include=FALSE}
# Create the second dataframe
data_df <- data.frame(
  result = c("Data not available", "Software not available", "Methods unclear", "Different result"),
  value = c(5, 1, 2, 2)
)
```
...
[https://nbis-reproducible-research.readthedocs.io/en/latest/feedback/](https://nbis-reproducible-research.readthedocs.io/en/devel/feedback/)

<iframe src="https://docs.google.com/forms/d/e/1FAIpQLScLBmhUGCq_6D_NMCTKtgz8k-5idJmPvKwZ_Ip3A_u8wy1WqQ/viewform?embedded=true" scrolling="yes" width="600" height="400" frameborder="0" marginheight="0" marginwidth="0" </iframe>
...
pie1_cols <- c("#4D702D","#A1C880","#F5E959","#E6A72E","#BA382F")
bp <- ggplot(df, aes(x="", y=value, fill=result)) + geom_bar(width = 1, stat = "identity", color = "white")  
pie1 <- bp + coord_polar("y", start = 0) + scale_fill_manual(values = pie1_cols) + blank_theme +
  theme(axis.text.x=element_blank(), legend.text=element_text(size=12), legend.position = 'left') + guides(fill = guide_legend(reverse = TRUE))
```
...
pie2_cols <- c("#E5876B","#BC5B33","#A2100C","#C8325E")
bp2 <- ggplot(data_df, aes(x="", y=value, fill=result)) + geom_bar(width = 1, stat = "identity", color = "white") 
pie2 <- bp2 + coord_polar("y", start=0) + scale_fill_manual(values = pie2_cols) + blank_theme +
  theme(axis.text.x=element_blank(),  legend.position = 'right', legend.text=element_text(size=12), legend.direction = "vertical", plot.margin= grid::unit(c(5,5,5,5), 'lines'))
```
.pull-right[
```{r, fig.height=6, echo=FALSE}
pie2
