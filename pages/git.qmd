...
::: {.callout-warning}
The directory should _not_ be within the `workshop-reproducible-research`
directory, since this is itself a Git-tracked directory.
:::

Once we are inside the desired directory, we can _initialise_ Git with the
following command:
...
that it is _not a git repository_. The way this works is that Git
...
there are currently no files being tracked. Let's add some fictitious script
that runs `bowtie` and an `environment.yml` file specifying some packages that
we need to install in order to be able to run the script:

```bash
echo "bowtie2 input > output " > bowtie.sh
printf "dependencies:\n  - bowtie2\n" > environment.yml
```
...
- Run `git status` again! See that we have added `bowtie.sh` to our upcoming
...
- Open up `bowtie.sh` in your favourite editor and add a `-x` flag to the
  command: the line should now say `bowtie2 -x input > output`.

- Run `git status`. It will tell you that there are modifications in one file
  (`bowtie.sh`) compared to the previous commit. This is nice! We don't have to
  keep track of which files we have edited, Git will do that for us.

- Run `git diff bowtie.sh`. This will show you the changes made to the file. A
  `-` means a deleted line, a `+` means an added line. A few lines before and
  after the changes are also shown for context.

- Let's edit the other file! Open `environment.yml` and change the second line
  to say `  - bowtie2=2.2.4`. Run `git status`, followed by `git diff`. If we
...
```bash
git add bowtie.sh environment.yml
```

This will add both our files to the staging area at the same time. Run `git
status` and see that the changes in both `bowtie.sh` and `environment.yml` are
ready to be committed.

### Unstaging files

But wait a minute! Shouldn't each commit optimally be a conceptual unit of
change? While all we have here is a toy example, one change is how we run a
command, while the other is about which version of packages we install, meaning
that these should probably be separate. We thus want to make two commits, one
for each change.
...
- Run `git status` again. See that now only `bowtie.sh` is staged for being
  committed, whereas the changes in `environment.yml` are tracked by Git, but
  not ready to be committed.

- Commit the changes in `bowtie.sh`:

```bash
git commit -m "Add -x flag to Bowtie command"
```
...
git add environment.yml
git status
git commit -m "Specify Bowtie version 2.2.4"
git status
```
...
git add environment.yml
git status
git commit -m "Remove environment.yml"
git status
```

Here we used `rm environment.yml` to delete the file and `git add
environment.yml` to stage the deletion. You can also use `git rm
environment.yml` to do both these operations in one step.
...
- Let's make some mock result files:
...
- Sometimes you want to ignore all files in a directory with one or two
  ...
  but there may be a smaller data file that you _do_ want to track, _e.g._
...
- Git allows you to ignore all files using the aforementioned wildcard, but
  then _exclude_ certain files from that ignore command. Open the `.gitignore`
...
- Add a fictitious `--very-sensitive-local` flag to the `bowtie.sh` script:
...
- Let's assume that the status of the repository as it is now is ready for
  ...
  used to generate the manuscript figures.
...
- Let's assume we now got comments from the reviewers, and by fixing those we
  had to update our code . Open `bowtie.sh` and remove the
  `--very-sensitive-local` flag (which reviewer \#3 didn't like). Commit and tag
  the changes:

```bash
git add bowtie.sh
git commit -m "Remove `--very-sensitive-local` flag"
git tag "revision-1"
```
...
- Open `bowtie.sh` and note that the is `--very-sensitive-local` flag is there!
...
- Open `bowtie.sh` and see that the flag is now gone.

::: {.callout-tip}
You can also see the difference between tags in the same way as for branches and
commits using _e.g._ `git diff <tag1> <tag2>`.
:::

At this point you could run `git log --oneline --decorate` to get a condensed
...
well, but for our cases here they are almost always the same.
:::
...
`origin/main` when you're on your local `main` branch.

- Set your upstream to `origin/main` using the command `git branch
...
This repository contains tutorial information related to the **NBIS/ELIXIR**
course _Tools for Reproducible Research_, specifically the session on using the
...
- We also learned the basics of _markdown_ and how it can be used to document
  Git repositories.
:::
...
- Go to your GitHub repository in your browser again and click on `bowtie.sh`.
  You will see the contents of the file. Notice that it is the latest version,
  where we previously removed the `--very-sensitive-local` flag:

> ![](images/github_bowtie_sh.png){ width=600px }

- Click on _History_. You will see an overview of the commits involving changes
  made to this file:

> ![](images/github_bowtie_sh_history.png){ width=600px }

- Click on the `Add --very-sensitive-local Bowtie flag` commit. You will see the changes
  made to `config.yml` file compared to the previous commit.

> ![](images/github_bowtie_sh_diff.png){ width=600px }
...
- Go back to the repository's main page and click on the commit tracker on the
  ...
  that commit. Click on the `Add initial files` commit, that was the initial
  commit.
...
you can see that the `environment.yml` is back, even though we deleted it! Click
on the name of the repository at the top of the page to return to the overview
of the latest repository version.
...
Let's say that we now want to add back the `environment.yml` file, as well as
add another piece of software: MultiQC with version `1.7`.

- Start with adding back the previously deleted `environment.yml` file:
...
- `cd` back into the first local repository (_i.e._ `git_tutorial`) and run the
...
repository as having one branch (you can see this by going to the repository's
main page). This is because we only pushed our `main` branch to the remote.
Let's create a new local branch and add some changes that we'll push as a
separate branch to our remote - you should do this in the original
`git_tutorial` repository, so make sure you're in that directory.

- Create a new branch named `trimming`, switch to it and add the `--trim5 5`
  flag to the Bowtie2 command in `bowtie.sh`, which should now look like this:
...
- Go the repository at GitHub and see if the new branch has appeared. Just above
  the file listing click the _Branch_ button and select the new branch to view
  it. Can you see the difference in the `bowtie.sh` depending on which branch
  you choose?
...
- Go to the repository overview page on GitHub. You will see that the repository
  ...
  the existing tags for your repository.
...
- Move into your other local repository, `git_remote_tutorial`, which doesn't
  have the new change. Run `git status`. Notice that Git says: "_Your branch is
  up-to-date with 'origin/main'._". We know that this is not true, but this
...
- Now run `git status`. Unlike before, our local Git clone now is aware of the
  ...
  lines: "_Your branch and 'origin/main' have diverged, and have 1 and
  1 different commit each, respectively._".
