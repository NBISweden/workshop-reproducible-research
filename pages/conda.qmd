...
`run_qc.sh`. This code will download the raw FASTQ-files and subsequently
...
- Now we can run the code for the MRSA project found in `run_qc.sh` by running
  `bash run_qc.sh`. Do this! (You can also open the file and run the commands
...
and results was `environment.yml` (that we used to create a Conda environment
with the required packages) and the analysis code in `run_qc.sh`.
