...
### Pixi, for a project-centric approach

Pixi is another package management tool that builds on the Conda ecosystem but
which has a more project-centric approach. Each 'project' is tied to a specific
folder that contains a `pixi.toml` manifest file describing the project. An
example of a `pixi.toml` file could look like this:
