...
we utilise _Jupyter notebooks_ in the tutorial after this one). Quarto itself
...
useful for scientific computing. The `engine` specifies which rendering engine
we want to use, and since we are going to be using R in this example, we specify
`knitr` here, which is the rendering engine used for R.
...
You can also specify parameters in a `params.yml` file and instruct quarto to
use them with the `--execute-params params.yml` flag when rendering. Note that
the parameters must be defined in the YAML header, pointing quarto to a
`params.yml` file with `--execute-params` only overrides them when rendering.

Using parameters is extremely useful when you're using a workflow manager system
(_e.g._ Snakemake or Nextflow, which we'll go through in subsequent tutorials),
since you can easily specify sample-specific parameters from the command line
directly from your workflow manager.
