...
```bash
snakemake --dag -c 1 a_b.txt | dot -Tpng > jobgraph.png
```
...
Let's add another parameter to the `get_SRA_by_accession` rule. As you can see
...
in the rule to use the parameter `max_reads` instead and set the value to 20000.
...
to would be `results/logs/<rule_name>/`. To avoid having multiple jobs write to
the same files Snakemake requires that all output and log files contain the same
wildcards, so be sure to include any wildcards used in the rule in the log name
as well, _e.g._ `{some_wildcard}.log`.
...
Now modify the rules `index_genome` and `align_to_genome` to use the `expand()`
expression in the output and input, respectively. Click the solution below if
you need help.
...
instead define a list of sample ids and put at the top of the Snakefile, just
before the rule `all`:
...
All of these issues can be dealt with by using the `shadow` directive for a
rule. This causes each execution of the rule to run in an isolated temporary
directory (located in `.snakemake/shadow/` by default). There are a few options
for `shadow` (for the full list of these options see the [Snakemake
...
begin with `rm`), as they are no longer needed. Check the solution below if you
need help.
...
logfiles, a config file etc. However, the workflow is not very general since
...
can run Snakemake workflows on UPPMAX where Apptainer is pre-installed.
...
will require 32G of RAM, while the `align_to_genome` rule will have a
runtime of 120 minutes and will require 24G of RAM. Both rules will use
...
There's a VSCode plugin for Snakefmt which you can use to automatically rewrite
...
Congratulations! You have now turned the MRSA workflow into a command-line tool (which runs Snakemake under the hood). You can even go one step further and skip the `snk install` step by directly building a [standalone workflow package](https://snk.wytamma.com/workflow_packages/).