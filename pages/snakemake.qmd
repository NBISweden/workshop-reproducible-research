...
Now try to run the whole workflow. As before you can specify `-p` to have
Snakemake print the shell commands for rules and you can control how many cores
to use with `-c`. If you don't specify number of cores Snakemake will use all
available cores on your machine. Let's run the workflow with one core:
...
divided into subdirectories. Take some time to look through the structure, in
particular the quality control reports in `results/multiqc/` and the count table
in `results/tables/`.
...
Now rerun the whole workflow with:
...
finished. Let's use the `temp` notation for for the output of `align_to_genome`, modify the rule so that the `output:` directive looks like this:
...
this with [list comprehensions](https://docs.python.org/3/tutorial/datastructures.html#list-comprehensions)
like this:
...
same thing with the built-in function `expand()`.
...
insert each of the values in the list `["1", "2", "3", "4", "rev.1", "rev.2"]`
in place of `{substr}`. The result is a list of strings that can be used as
input to the rule. This is a very powerful feature that can save you a lot of
...
`results/tables/counts.tsv`, are in turn specified as input to the `all` rule at
the top of the file. Because the first rule is targeted by default when we run
Snakemake on the command line (like we mentioned in the [MRSA workflow
section](#the-mrsa-workflow)) this is what triggers the rules to run on each of
the three samples.
...
ule generate_count_table:
    ...
    output:
        "results/tables/counts.tsv"
    input:
        bams = expand("results/bam/{sample_id}.sorted.bam", sample_id = SAMPLES),
...
- (Contain any other parameters we might need to make it into a general
  workflow, _e.g._ to support both paired-end and single-read sequencing)
...
Remove the line with `SAMPLES = ["SRR935090", "SRR935091", "SRR935092"]` that we
added to the top of `snakefile_mrsa.smk` in the [targets section](#targets).

Do a dry-run afterwards to make sure that everything works as expected.

You may remember from the [parameters section](#parameters) part of this
tutorial that we're using a function to return the URL of the FASTQ files to
download for each sample:
...
_i.e._ only expanding the wildcard referring to the Bowtie2 index. To keep the
