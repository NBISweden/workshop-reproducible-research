...
top of the panel then clicking "Rename Notebook". Enter "jupyter-tutorial.ipynb".
...
Let's use our first cell to create a header. Change the format from Code to by
pressing the `m` key when in command mode. Click inside the cell and input `# My
notebook`. Run the cell with `ctrl`-`enter` (`cmd`-`enter` on Mac).
...
```bash
jupyter nbconvert --ExecutePreprocessor.kernel_name=jupyter-env --execute --to HTML --output supplementary_material.nbconvert.html supplementary_material.ipynb
```

::: {.callout-note}

The `--ExecutePreprocessor.kernel_name=jupyter-env --execute` flags tell
`nbconvert` to run the notebook using the `jupyter-env` kernel.
...
document options that Quarto understands. Create a new cell at the very top of
the notebook (from the Jupyter lab interface). In this cell, add the following:
