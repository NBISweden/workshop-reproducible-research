...
Seems like I'm running the Darwin version of MacOS.
...
and then prepend the command with `docker run <image>`. First of all we need to
download the genome to index, though.
...
`quay.io/biocontainers/bowtie2:2.5.1--py39h3321a2d_0` is the IMAGE but instead
of first downloading and then running it we point to its remote location
directly, which will cause Docker to download it on the fly. The `bowtie2-build`
part is the COMMAND followed by the ARG (the input tempfile and the output
index)
...
- Build an image using the following command: `docker build -f slim.Dockerfile
  -t my_docker_image .`.
...
- Execute the `run_qc.sh` script inside the container with the appropriate bind
  mounts using `docker run -v $(pwd):/analysis my_docker_conda
  /analysis/run_qc.sh`.
...
- Let's try to run a container from the image we just created by running `docker
  run my_docker_conda`.`
...
- Start a new container in detached mode using `docker run -d --rm --name
  my_container my_docker_conda`.
...
`slim.Dockerfile`, triggered by changes made to the GitHub repository. You can
take a look at the course on [Docker Hub](https://hub.docker.com/r/nbisweden/workshop-reproducible-research).
:::
...
- Convert the Docker image for this course directly from DockerHub using the
  following command: `apptainer pull mrsa_proj.sif docker://nbisweden/workshop-reproducible-research`.
...
`code/supplementary_material.qmd`).

- Execute the workflow inside Apptainer using `apptainer run mrsa_proj.sif`.

This executes the default run command, which is `snakemake -rp -c 1 --configfile
config.yml` (as defined in the original `Dockerfile`), and since Apptainer
automatically bind mounts the current working directory we have access to all
the required input files already. Once completed you should see a bunch of
directories and files generated in your current working directory, including the
`results/` directory containing the final HTML report.
...
- Copy the above code into _e.g._ `base.Dockerfile` and build it using `docker
  build -f base.Dockerfile -t my_docker_base .`
...
- Now check the difference between the previous `my_docker_project` and your new
  image. How much of a size did you get?
