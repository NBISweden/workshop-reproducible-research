...
`containers` directory (where you should hopefully be standing already). We will
now go through that file and discuss the different steps and what they do. After
that we'll build the image and test it out. Lastly, we'll start from that image
and make a new one to reproduce the results from the [Conda
...
- Build an image using the following command: `docker build -f Dockerfile -t
  my_docker_image .`.
...
- Convert the Docker image for the `lolcow` software directly from the GitHub
  Container Registry using the following command: `apptainer pull lolcow.sif
  docker://ghcr.io/apptainer/lolcow`.
