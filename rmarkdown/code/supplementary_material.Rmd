...
All files needed to reproduce this analysis are available in a Bitbucket repository. There is a Snakemake workflow that can be used to rerun the full analysis from downloading data to producing this Supplementary Material. A Conda environment can be used to install all needed dependencies. Furthermore, a Docker image is available that contains a prepared environment for directly running the analysis in an isolated system. The figures in this Supplementary were generated in the following R environment:
...
knitr::kable(meta[,columns], caption="QC stats from FastQC")
```

```{r counts-barplot, fig.cap=paste("Counting statistics per sample, in terms of read counts for genes and reads not counted for various reasons. ")}
...
pheatmap(log10(d[,-1]+1), labels_row=paste(rownames(d)," (",d$description,")",sep=""), fontsize_row=8)
```

```{r rulegraph, fig.cap=paste("A rule graph showing the different steps of the bioinformatic analysis that is included in the Snakemake workflow."), out.height="11cm"}
