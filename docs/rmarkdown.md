...
* Add the parameters listed below to the YAML header. These correspond to files (and sample IDs) that are generated by the MRSA analysis workflow (see the [Snakemake tutorial](snakemake.md)) and contain results that we want to include in the Supplementary material document:
