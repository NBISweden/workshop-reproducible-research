...
# Read counts:
counts <- read.delim("data/final/counts.tsv")
...
gse <- as.data.frame(getGEO("GSE48896", GSEMatrix=T)[[1]])
gsm2srr <- data.frame(geo_accession=c("GSM1186459", "GSM1186460", "GSM1186461"),
                      SRR=c("SRR935090", "SRR935091", "SRR935092"))
meta <- transform(merge(x=gse, y=gsm2srr, by.x="geo_accession",by.y="geo_accession"), row.names=SRR)
qc <- read.delim("intermediate/multiqc_general_stats.txt")
...
# Read GFF:
gff <- as.data.frame(readGFF("data/raw_external/NCTC8325.gff3.gz"))
```
...
knitr::include_graphics(normalizePath("results/rulegraph.png"))
```
