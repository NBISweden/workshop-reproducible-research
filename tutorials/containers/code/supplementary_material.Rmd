...
# Read counts:
counts <- read.delim(counts_file, header=T, comment.char = "#", row.names=1)
counts <- counts[, 5:ncol(counts)]
colnames(counts) <- gsub(".*(SRR[0-9]+)\\..*", "\\1", colnames(counts))
# Read summary file
counts_other <- read.delim(summary_file, sep = "\t", header = T, row.names = 1)
colnames(counts_other) <- gsub(".*(SRR[0-9]+)\\..*", "\\1", colnames(counts_other))
...
```{r Counts barplot}
count_data <- rbind(genes = apply(counts[, -1], 2, sum), counts_other[, -1])
...
labels_row <- paste0(rownames(heatmap_data), " (",
                     ...
                     ")")
pheatmap(log10(heatmap_data[, -1] + 1),
         fontsize_row = 8,
